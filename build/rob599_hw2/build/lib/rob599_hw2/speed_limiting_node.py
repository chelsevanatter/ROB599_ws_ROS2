...
class SpeedLimitingNode(Node):
	...
	def callback(self, msg, linear_max, angular_max):
		...
		if abs(msg.linear.x) > linear_max:
			msg.linear.x = linear_max if msg.linear.x > 0 else -linear_max
		if abs(msg.linear.y) > linear_max:
			msg.linear.y = linear_max if msg.linear.y > 0 else -linear_max
		if abs(msg.linear.z) > linear_max:
			msg.linear.z = linear_max if msg.linear.z > 0 else -linear_max
		...
		if abs(msg.angular.x) > angular_max:
			msg.angular.x = angular_max if msg.angular.x > 0 else -angular_max
		if abs(msg.angular.y) > angular_max:
			msg.angular.y = angular_max if msg.angular.y > 0 else -angular_max
		if abs(msg.angular.z) > angular_max:
			msg.angular.z = angular_max if msg.angular.z > 0 else -angular_max
